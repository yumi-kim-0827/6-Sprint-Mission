...
export function ItemDetailPage() {
  ...
  const [isItemDetailLoading, itemDetailLoadingError, getItemDetailAsync] = useAsync(getItemDetail);
  const [isItemCommentLoading, itemCommentLoadingError, getItemCommentAsync] = useAsync(getItemComments);

  const handleLoad = useCallback( async (productId) => {
    let productResult = await getItemDetailAsync(productId);
    let commentResult = await getItemCommentAsync(productId);
  ...
  return (
      ...
      <div className="section-content">
        <div className="section-row">
          ...
          <section className="section-detail-content">
            ...
            <div className="section-content"><p>{product.description}</p></div>
          </section>
          <section className="section-detail-content">
            ...
            <div className="section-content tag-view">
              <ul className="tag-container">
                {tags.map((tag) => {
                  return (
                    <li className="tag-view__list">{tag}</li>
                  );
                })}
              </ul>
            </div>
    ...
    <section className="section-comment">
      ...
      <div className="section-content">
        <textarea name="comment" className="input-theme txt-comment" placeholder="개인정보를 공유 및 요청하거나, 명예 훼손, 무단 광고, 불법 정보 유포시 모니터링 후 삭제될 수 있으며, 이에 대한 민형사상 책임은 게시자에게 있습니다."/>
        <button type="button" className="btn-small btn-comment">등록</button>
      </div>  
    </section>
    <section className="section-reply">
      {comments ?? <ul className="reply-list">
