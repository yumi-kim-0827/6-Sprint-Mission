...
import PageNav from "pages/item/Products/component/PageNav";
...
const Products = ({ numOfItemsToShow }: { numOfItemsToShow: number }) => {
  ...
  //페이지네이션 숫자 설정
  const settingPageNumbers = (totalPageCount: number) => {
    if (totalPageCount === 0) {
      return;
    }
    //5번까지 없을 경우
  ...
  //정렬 선택하기
  const handleSelectOption = (selectedOrder: string) => {
