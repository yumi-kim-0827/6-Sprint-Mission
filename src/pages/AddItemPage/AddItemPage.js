...
import addFile from '../../assets/images/icons/addfile.png';
import tagdelete from '../../assets/images/icons/deletetag.png';
...
function ProductTag({ name, value, onChange, clearProductTag }) {
  ...
  const handleDeleteTag = (e) => {
    ...
    const compareValue = tagValue.clice(2, tagValue.length);
      ...
      <div className="show-tag">
        {tagArr.map((tag, idx) => {
          return (
            <p key={idx}>
              {`#${tag}`}
              <img src={tagdelete} alt="태그 삭제" onClick={handleDeleteTag} />
            </p>
          );
        })}
      </div>
...
const AddItems = () => {
  ...
  const clearProductTag = (value) => {
    setProductValues((prevProductValues) => {
      const updatedProductValues = prevProductValues.productTag.filter(
        (tag) => tag !== value
      );
      return {
        ...prevProductValues,
  ...
  return (
    <form className="product-form" onSubmit={handleSubmit}>
      ...
      <ProductImg
        name="productImg"
        value={productValues.productImg}
      ...
      <ProductTag
        name="productTag"
        value={productValues.productTag}
        onChange={handleChange}
        clear
        ProductTag={clearProductTag}
      />
    </form>
  );
};
