...
type SelectMouseEvent = MouseEvent<HTMLLIElement>;
type ButtonMouseEvent = MouseEvent<HTMLLIElement>;

function SelectMenu({ setOrder }) {
  const [isOpen, setIsOpen] = useState(false);

  const handleButtonClick = useCallback((e: ButtonMouseEvent) => {
    e.stopPropagation();
    setIsOpen((nextIsOpen) => !nextIsOpen);
  }, []);

  const handleSelect = (order: string, e: SelectMouseEvent) => {
